                ...
                if changes:
                    ...
                    for change in changes:
                        print(
                            ...
                            f"{change.get('type'):<25} | {change.get('owner'):<18} |"
                            f"{change.get('active_buy_count'):>3} | {change.get('buy_range')}",
                        )
...
async def handle_exception(exception, analyzer, change):
    ...
    if "{'code': 1898}" in error_message:
        print("Ошибка: НЕТ СВОБОДНЫХ СЛОТОВ ДЛЯ ПРОДАЖИ")
...
if __name__ == "__main__":
    while True:
        try:
            asyncio.run(main("email@mail.ru", "password!", sell_price=13254))
        except Exception as e:
