...
class AsyncUbisoftMarketClient:
    ...
    async def create_sell_order(
        self, space_id: str, item_id: str, quantity: int, price: int
    ) -> Dict:
        mutation = """
        ...
        try:
            ...
            self.db.insert_sell_order(
                {
                    "space_id": space_id,
                    "trade_id": result.get("createSellOrder")
                    .get("trade")
                    .get("tradeId"),
                    "item_id": item_id,
                    "quantity": quantity,
    ...
    async def get_sellable_items(
        ...
        variables = {
            ...
            "filterBy": {"types": item_types or [], "tags": tags or []},
            "sortBy": {
                "field": sort_field,
                "orderType": "Sell",
    ...
    def parse_market_data(self, response: Dict) -> List[Dict]:
        ...
        try:
            ...
            for node in nodes:
                ...
                sell_stats = (
                    market_data.get("sellStats")[0]
                    if market_data.get("sellStats")
                    else {}
                )
                last_sold = (
                    market_data.get("lastSoldAt")[0]
                    if market_data.get("lastSoldAt")
                    else {}
                )
                buy_stats = (
                    market_data.get("buyStats")[0]
                    ...
                    parsed_item = {
                        ...
                        "market_info": {
                            ...
                            "last_sold_at": datetime.fromisoformat(
                                last_sold.get("performedAt").replace("Z", "+00:00")
                            ),
    ...
    @staticmethod
    def parse_market_data_extended(response: Dict) -> List[Dict]:
        items = []
        try:
                        ...
                        "last_sold": {
                            ...
                            "performed_at": (
                                datetime.fromisoformat(
                                    last_sold.get("performedAt", "").replace(
                                        "Z", "+00:00"
                                    )
                                )
                                if last_sold.get("performedAt")
                                else None
                            ),
                        },
                    },
                    "active_trade": (
                        {
                            ...
                            "created_at": (
                                datetime.fromisoformat(
                                    active_trade.get("createdAt", "").replace(
                                        "Z", "+00:00"
                                    )
                                )
                                if active_trade.get("createdAt")
                                else None
                            ),
                            "expires_at": (
                                datetime.fromisoformat(
                                    active_trade.get("expiresAt", "").replace(
                                        "Z", "+00:00"
                                    )
                                )
                                if active_trade.get("expiresAt")
                                else None
                            ),
