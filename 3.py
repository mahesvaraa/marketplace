...
class UbisoftMarketClient:
    ...
    async def authenticate(self) -> None:
        ...
        if self.key and self.expiration:
            ...
            if expiration_time > datetime.now(timezone.utc):
                self._update_client()  # Добавляем инициализацию клиента даже если токен валиден
                return
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if "ticket" in data:
                ...
                self.sessionid = data.get("sessionId")
                self.userid = data.get("userId")
                self._update_client()  # Инициализируем клиента после получения новых данных
            else:
    ...
    async def create_sell_order(self, space_id: str, item_id: str, quantity: int, price: int) -> Dict:
        ...
        # Используем именованную операцию в mutation
        mutation = gql("""
        ...
        try:
            ...
            if time.time() - self._last_auth_time > self.auth_refresh_interval:
                await self.authenticate()
                self._update_client()  # Обновляем клиент после переавторизации

            result = self.client.execute(mutation, variable_values=variables)
            return result
        except Exception as e:
            print(f"Ошибка при создании ордера на продажу: {str(e)}")
            # Если получаем ошибку авторизации, пробуем переавторизоваться и повторить запрос
